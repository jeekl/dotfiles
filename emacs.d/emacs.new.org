...
Just override to make undo work like everywhere else...
#+begin_src emacs-lisp
(global-set-key (kbd "C-z") 'undo)
#+end_src
...
;r(eval-after-load "whole-line-or-region-mode" (diminish 'whole-line-or-region-mode))
...
TODO: This for some reason, does not show the line on startup. removing it and reenabling it fixes it though. Strange.
...
*** Keybinding
#+begin_src emacs-lisp
(global-set-key (kbd "C-x =") 'my-diff-current-buffer-with-disk)
#+end_src

** Backups / autosaves and so on
Settings for Emacs autosave, backup and similar systems. Basically they litter your system with useless files everywhere. :-/
#+begin_src emacs-lisp
